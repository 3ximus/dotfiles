...
function usage(){
	...
	echo "Usage:"
	echo "	 $0 [-i] [command | script]"
	echo "Options:"
	echo "	 -i    Run with attached terminal, instead of capturing output"
...
cron_env="$home/.cache/run-as-cron/cron-env"
...
for arg in "$@"; do
	if [ "$arg" == "-i" ]; then
		continue # ignore option to run interactively
	fi
...
if [ "$1" == "-i" ]; then
	"$the_shell" -c "$env_string $cmd_string"
else
