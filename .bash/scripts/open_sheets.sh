...
# the first argument is the name of the file, if omited 'temp' is given
# NOTE Depends on ./extract_cookies.sh script

$(dirname ${BASH_SOURCE[0]})/extract_cookies.sh > /tmp/sheets_upload_cookies.txt
...
{"title":"$1","mimeType":"application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"}
--lygsa2ob8du8
...
FILEID=$(curl 'https://clients6.google.com/upload/drive/v2internal/files?uploadType=multipart&supportsTeamDrives=true&fields=title%2Cid&pinned=false&convert=true&openDrive=false&reason=907&syncType=0&errorRecovery=false&key=AIzaSyAw-cTyp9Xotzvu3vNDWhDU3E9NConkKxQ' \
	-X POST \
